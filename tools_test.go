...
func TestTools_PushJSONToRemote(t *testing.T) {
	...
	if err != nil {
		t.Error("PushJSONToRemote failed:", err)
	}
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		testTool.AllowUnknownField = e.allowUnknown
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	err := testTool.ErrorJSON(rr, errors.New("foo"), http.StatusServiceUnavailable)
	if err != nil {
		t.Error("ErrorJSON failed:", err)
	}
	...
	if !payload.Error {
		t.Error("Error set to false in JSON, should be true", err)
	}

	if rr.Code != http.StatusServiceUnavailable {
		t.Error("Status code set to", rr.Code, "should be", http.StatusServiceUnavailable)
	}
}
